...
import { updateFromPlace } from '../../Actions/actions';
...
const SearchPlace = (props) => {
  ...
  const handleChange = add => {
    setaddress(add);
    store.dispatch(updateFromPlace({ description: add, placeId: "", types: [], place_location: { lat: undefined, lng: undefined } }));
  ...
  const handleSelect = async (address, place_id) => {
    ...
    if (place_id !== null) {
      ...
      suggestions_arr.map(suggestion => {
        if (suggestion.placeId === place_id) { place = suggestion; }
        return null;
      })

      let promise = geocodeByAddress(address)
        .then(results => {
          console.log(results); handleChange(place.description);
            ...
            <div className="autocomplete-dropdown-container">
              {loading && <div>Loading...</div>}
              {
                ...
                suggestions.map(suggestion => {
                  ...
                  const style = suggestion.active ? { backgroundColor: '#ffaaaa', cursor: 'pointer' } : { cursor: 'pointer' };
                  return (
                    <div
